...
def generate_images(
        ...
        increment: Optional[float] = 0.01,
        random_seed: Optional[int] = 0,
        diameter: Optional[float] = 100.0,
        scale_type: Optional[str] = "pad",
        size: Optional[List[int]] = None,
        seeds: Optional[List[int]] = None,
        space: str = "z",
        fps: Optional[int] = 24,
        frames: Optional[int] = 240,
    ...
    if (process == 'image'):
        ...
        for seed_idx, seed in enumerate(seeds):
            ...
            # v_range = 20
            # i_range = 10
            # im = PIL.Image.new('RGB', (1024 * i_range, 1024 * v_range))
            # x_offset = 0
            # y_offset = 0
            ...
            #         img = G(z, label, truncation_psi=truncation_psi, noise_mode=noise_mode)
            #         img = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
            #         im.paste(PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB'), (x_offset, y_offset)) #.save(f'{outdir}/seed{seed:04d}-{vector}-{i}.png')
            #
            # im.save(f'{outdir}/vectors-{seed}.jpg')
